odoo.define('hispark_pos_combo.OwnCombo', function (require) {
    "use strict";
    ...
    class OwnComboWidget extends AbstractAwaitablePopup {
        setup() {
            super.setup();
            var self = this;
        ...
        clickProduct(){
            ...
            if(self.counter > 0){
                ...
                $(".pos_top_product").click(function(){
        ...
        display_order_line(order_menu){
            ...
            html_text += "<tbody style='font-size: 16px; justify-content: center;'>";
            var total_price = 0;
            for(var i=0; i<order_menu.length; i++){
                html_text += "<tr><td style='float:left; margin:10px;'><b style='font-size: 20px;color: black'>"+order_menu[i].categoryName+"</td></b><td></td></tr>";
                for(var j=0; j<order_menu[i].products.length; j++){
                    html_text += "<tr><td style='float:left;margin-left: 10px;margin-bottom:10px;'>"+order_menu[i].products[j].product_name+"</td><td style='float:right;margin-right: 10px;'>"+(order_menu[i].products[j].price.toFixed(2))+"</td></tr>";
    ...
    OwnComboWidget.template = 'OwnComboWidget';
    OwnComboWidget.defaultProps = { cancelText: _lt('Cancel'),title: 'Make own  Combo', confirmText: _lt('Ok') };
    Registries.Component.add(OwnComboWidget);
});
