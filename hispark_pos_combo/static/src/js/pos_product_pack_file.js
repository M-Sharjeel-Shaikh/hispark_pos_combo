odoo.define('hispark_pos_combo.pos_product_pack_file', function (require) {
    "use strict";
        ...
        export_as_JSON (){
            ...
            if (this.product.is_extra && this.own_data) {
                _.each(this.own_data, function(item) {
                    own_line.push([0, 0, {
                        product_id: item.product_id.id || item.product_id, // Use ID
                        name: item.product_id.display_name || item.product_id,
            ...
            if (this.product.is_extra) {
                ...
                json.own_ids = JSON.stringify(own_line);
            } else {
